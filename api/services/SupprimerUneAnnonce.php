...
{	$msg = "Erreur : methode HTTP incorrecte.";
    ...
    {	// il faut etre administrateur pour supprimer un utilisateur
        if ( $dao->getConnexionToken($token) != true)
    	...
    	{	// contrôle d'existence de pseudoAsupprimer
    	    ...
    	    if ($uneAnnonce == null || $dao->getUtilisateur($token)->getId() != $uneAnnonce->getUtilisateur()->getId())
    	    {  $msg = "Erreur : Annonce inexistante ou non possédé.";
    	       $code_reponse = 400;
    	    }
    	    else
    	    { 
