...
$idAnnonce = ( empty($this->request['idAnnonce'])) ? "" : $this->request['idAnnonce'];
...
{	$msg = "Erreur : méthode HTTP incorrecte.";
    ...
    {	if ( !$dao->getConnexionToken($token) ) {
    	...
    	{	// récupération de la liste des utilisateurs à l'aide de la méthode getTousLesUtilisateurs de la classe DAO
    	    $uneAnnonce = $dao->getUneAnnonce($idAnnonce);
...
if ($lang == "xml") {
    ...
    $donnees = creerFluxXML($msg, $uneAnnonce);
}
else {
    $content_type = "application/json; charset=utf-8";      // indique le format Json pour la réponse
    $donnees = creerFluxJSON($msg, $uneAnnonce);
}
...
function creerFluxJSON($msg, $uneAnnonce)
{
    ...
    $elt_annonce = ["Annonce" => $unObjetAnnonce];
